...
export const getAlternativeFor = (exerciseId: string): string | null => ALTERNATIVES[exerciseId] ?? null;

export function getExerciseConflict(exerciseId: string, injuries: ExerciseInjury[]): ExerciseConflict | null {
  ...
  for (const injury of injuries.filter((item) => !item.clearedDate)) {
    const tags = exercise.tags.filter((tag) => injury.forbiddenTags.includes(tag));
    if (tags.length > 0) {
      return {
        injuryId: injury.id,
        injury: injury.name,
...
export function getBlockedExercises(injury: ExerciseInjury): string[] {
  return EXERCISE_LIBRARY
    .filter((exercise) => exercise.tags.some((tag) => injury.forbiddenTags.includes(tag)))
