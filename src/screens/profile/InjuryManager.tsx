      ...
      <div className="space-y-3">
        {active.map((injury) => (
          <div key={injury.id} className="flex items-start justify-between border border-[#1a1a1a] bg-[#0c0c0c] p-4">
            <button onClick={() => onEdit(injury)} className="space-y-2 text-left">
              <div className="flex items-center space-x-2">
                <span className={`border px-1.5 font-mono text-[8px] font-bold uppercase ${injury.severity === 'avoid' ? 'border-red-900 bg-red-950 text-red-400' : 'border-yellow-900 bg-yellow-950 text-yellow-500'}`}>
              ...
              <div className="flex flex-wrap gap-1 pt-1">
                {injury.forbiddenTags.map((tag) => (
                  <span key={tag} className="border border-[#1a1a1a] bg-black px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider text-neutral-400">
                    {tag.replace(/_/g, ' ')}
                  </span>
                ))}
              </div>
        ...
        {active.length === 0 && (
          <div className="border border-dashed border-[#1a1a1a] bg-black p-6 text-center">
            <span className="font-mono text-xs uppercase tracking-widest text-neutral-500">No injuries logged</span>
          </div>
        )}
      </div>
      {past.length > 0 && (
        <div className="space-y-2">
          ...
          {showPast && past.map((injury) => (
            <div key={injury.id} className="border border-[#1a1a1a] bg-black p-3 font-mono text-[10px] text-neutral-500">
