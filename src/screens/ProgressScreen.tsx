...
} from "@/domain/progress/consistency";
import { buildMesocycleSummaries } from "@/domain/progress/cycles";
import {
  buildExerciseBests,
  detectProgressRecords,
} from "@/domain/progress/exercises";
...
import {
  ProgressTabs,
  type ProgressTab,
} from "@/screens/progress/ProgressTabs";
...
export function ProgressScreen() {
  ...
  const model = useMemo(() => {
    ...
    const cycleSource = activeProgram
    ...
    return {
      ...
      muscles: buildMuscleProgress(
        workouts,
        today,
        28,
        activeProgram?.musclePriorities,
      ),
      cycles: buildMesocycleSummaries(cycleSource, workouts, records),
      responses: buildExerciseResponseHistory(decisions, customExercises),
    };
  }, [
    activeProgram,
    customExercises,
    ...
    <div className="animate-rise-in space-y-5">
      <header>
        ...
        <p className="mt-2 max-w-xl text-sm leading-6 text-text-secondary">
          Review completed training, exercise performance, muscle exposure, records, and
          cycle response from your own history.
        </p>
      </header>
