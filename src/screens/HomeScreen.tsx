...
export function HomeScreen({ setActiveTab }: HomeScreenProps) {
  ...
  const start = () => {
    ...
    if (recovery?.plannedSessionId === session.id) {
      ...
      hydrateProgramData(
        programs,
        mesocycles.map((cycle) => (cycle.id === repaired.id ? repaired : cycle)),
      );
      session =
        repaired.weeks
  ...
  return (
    <div className="animate-rise-in space-y-5">
      ...
      {cycleComplete ? (
        ...
        <section className="surface-raised p-5 sm:p-6">
          <p className="text-sm font-semibold text-signal-strong">Set up training</p>
          <h2 className="mt-1 text-2xl font-bold tracking-[-0.03em] text-text">
            Create your program
          </h2>
          <p className="mt-2 text-sm leading-6 text-text-secondary">
            Choose a sustainable weekly schedule and training priorities. Macht will turn
            that definition into the sessions shown here.
          </p>
          <Button onClick={() => setActiveTab("templates")} className="mt-5 w-full">
            Open Program
          </Button>
        </section>
      ) : programNeedsCycle || !next ? (
        <section className="surface-raised p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-text-secondary">
            Your program definition is saved. Generate a cycle to place its prescriptions
            on Today.
          </p>
          <Button onClick={() => setActiveTab("templates")} className="mt-5 w-full">
            Open Program
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-sm text-text-secondary">
                ...
                <span className="inline-flex items-center gap-1.5">
                  ...
                  ≈{next.session.targetDurationMinutes} min
                </span>
                <span className="inline-flex items-center gap-1.5">
