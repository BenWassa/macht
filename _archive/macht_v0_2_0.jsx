...
function getPlates(weight, units = "lbs") {
  ...
  const denoms =
    units === "lbs" ? [45, 35, 25, 10, 5, 2.5] : [25, 20, 15, 10, 5, 2.5, 1.25];
  const result = [];
...
export default function App() {
  ...
  const getExerciseConflict = (exId) => {
    ...
    for (const injury of injuries) {
      const hits = exercise.tags.filter((t) =>
        injury.forbiddenTags.includes(t),
      );
      if (hits.length > 0) {
        return {
          injury: injury.name,
          ...
          alternative: getAlternativeFor(exId),
        };
      }
    }
    return null;
  ...
  const handleTweak = (field, amount) => {
    ...
    updateSetField(
      ...
      selectedSetIndex,
      field,
      Number.isInteger(next) ? next : parseFloat(next.toFixed(1)),
    );
  };
  ...
  const saveWorkout = () => {
    const totalSets = activeWorkoutList.reduce(
      (acc, exId) =>
        acc + (workoutSets[exId]?.filter((s) => s.completed).length || 0),
      0,
    );
  ...
  const toggleTagInForm = (tag) => {
    ...
    setInjuryForm({
      ...injuryForm,
      forbiddenTags: tags.includes(tag)
        ? tags.filter((t) => t !== tag)
        : [...tags, tag],
    });
  };
    ...
    <div className="min-h-screen bg-[#060606] text-[#f0f0f0] flex flex-col font-sans antialiased selection:bg-blue-600 selection:text-white">
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="bg-[#0c0c0c] border border-[#1a1a1a] p-4">
              ...
              <div className="grid grid-cols-6 gap-2 h-24 items-end border-b border-[#1a1a1a] pb-2 relative">
                <div
                  className="absolute left-0 right-0 border-t border-dashed border-neutral-800"
                  style={{ bottom: "33.3%" }}
                >
                  <span className="absolute -top-2 right-0 text-[8px] font-mono text-neutral-500 bg-[#0c0c0c] pl-1">
                    Floor: 2
                  </span>
                </div>
                <div
              ...
              <div className="grid grid-cols-6 gap-2 pt-2 text-center">
                {WEEK_CONSISTENCY.map((w, idx) => (
                  <span
                    key={idx}
                    className="text-[9px] font-mono text-neutral-500 uppercase"
                  >
                    {w.week}
                  </span>
                ))}
            ...
            <div className="space-y-3">
              ...
              <div className="border border-[#1a1a1a] divide-y divide-[#1a1a1a]">
                {history.map((log) => (
                  <div
                    key={log.id}
                    className="bg-[#0c0c0c] p-4 flex items-center justify-between"
                  >
                    <div>
                      ...
                      <h4 className="text-xs font-bold uppercase tracking-tight text-neutral-200 mt-1">
                        {log.template}
                      </h4>
                      <div className="flex space-x-3 text-[10px] font-mono text-neutral-500 mt-1">
                        <span>
                          Vol. {log.volume.toLocaleString()} {settings.units}
                        </span>
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="bg-[#0c0c0c] border border-[#1a1a1a] p-5 space-y-4">
              <div>
                ...
                <h3 className="text-sm font-bold uppercase tracking-tight mt-2.5">
                  {DEFAULT_TEMPLATE.name}
                </h3>
                <p className="text-[11px] text-neutral-500 mt-0.5">
                  {DEFAULT_TEMPLATE.notes}
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <span className="text-xs font-semibold uppercase tracking-tight text-neutral-300">
                          {ex?.name}
                        </span>
                      </div>
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="flex justify-between items-start border-b border-[#1a1a1a] pb-4">
              <div>
                ...
                <h1 className="text-lg font-bold uppercase tracking-tight font-mono">
                  {workoutName}
                </h1>
            ...
            <div className="flex items-center justify-between bg-[#0c0c0c] border border-[#1a1a1a] px-4 py-2.5">
              <div>
                ...
                <p className="text-[10px] text-neutral-600 mt-0.5">
                  Still counts toward consistency
                </p>
              </div>
              <button
              ...
              >
                <span
                  className={`absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform ${isMinimumSession ? "translate-x-5" : "translate-x-0.5"}`}
                />
              </button>
            </div>
                    ...
                    <div className="border border-red-900 bg-red-950/20 p-4 space-y-3">
                      <div className="flex items-start space-x-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-neutral-400 text-[11px] mt-0.5">
                            {exercise.name} conflicts with{" "}
                            <strong className="text-neutral-200">
                              {conflict.injury}
                            </strong>
                            . Flagged: {conflict.tags.join(", ")}.
                          </p>
                        </div>
                      </div>
                      {conflict.alternative &&
                        (() => {
                          const altEx = EXERCISE_LIBRARY.find(
                            (e) => e.id === conflict.alternative,
                          );
                          return (
                  ...
                  <div className="bg-[#0c0c0c] border border-[#1a1a1a] p-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div>
                        ...
                        <h2 className="text-sm font-bold uppercase tracking-tight mt-0.5">
                          {exercise.name}
                        </h2>
                      </div>
                      <div className="bg-black border border-[#1a1a1a] px-3 py-2 shrink-0">
                        <span className="text-[8px] font-mono text-blue-400 uppercase block mb-0.5 tracking-wider">
                          Last time
                        </span>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="col-span-2 text-left flex items-center space-x-2">
                              <span
                                className={`text-[10px] font-mono font-bold ${isActive ? "text-blue-400" : "text-neutral-500"}`}
                              >
                                {idx + 1}
                              </span>
                              {isActive && (
                                <span className="text-[8px] font-mono bg-blue-950 text-blue-400 px-1 border border-blue-900">
                                  ACTIVE
                                </span>
                              )}
                            </div>
                            <div className="col-span-3 text-center">
                              <span
                                className={`text-xs font-mono font-bold ${isActive ? "text-blue-300" : "text-neutral-300"}`}
                              >
                                {set.weight}{" "}
                            ...
                            <div className="col-span-2 flex justify-center">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleToggleComplete(currentExId, idx);
                                }}
                  ...
                  <div className="bg-[#0c0c0c] border border-[#1a1a1a] p-4 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-6">
                      ...
                      <div className="flex-1 bg-black border border-[#1a1a1a] h-24 relative flex items-center px-4 overflow-hidden">
                        ...
                        <div
                          className="absolute left-8 w-2 bg-[#444] border-x border-[#555] z-10"
                          style={{ height: 72 }}
                        />
                        <div
                          className="absolute left-11 flex items-center z-20"
                          style={{
                            gap: 2,
                            height: "100%",
                            alignItems: "center",
                        ...
                        >
                          {getPlates(selectedSet.weight, settings.units).map(
                            (plateVal, idx) => {
                              const data = (
                                settings.units === "lbs"
                                  ? PLATE_DATA_LBS
                          ...
                          {getPlates(selectedSet.weight, settings.units)
                            .length === 0 && (
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="space-y-6">
              {Object.keys(PROGRESS_LIFTS).map((key) => {
                ...
                const points = lift.history
                  .map((val, idx) => {
                    const x = (idx / (lift.history.length - 1)) * 360 + 20;
                    const y = val
                      ? 80 - ((val - minVal) / (maxVal - minVal)) * 60
                      : null;
                ...
                return (
                  <div
                    key={key}
                    className="bg-[#0c0c0c] border border-[#1a1a1a] p-5 space-y-4"
                  >
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b border-[#1a1a1a] pb-3 gap-2">
                      <div>
                        <h3 className="text-sm font-bold uppercase tracking-tight text-neutral-200 font-mono">
                          {lift.name}
                        </h3>
                        <p className="text-[9px] font-mono uppercase text-neutral-500 mt-0.5">
                      ...
                      <div className="flex space-x-6 text-right">
                        ...
                        <div>
                          ...
                          <span
                            className={`text-sm font-mono font-bold block ${lift.paused ? "text-neutral-500" : "text-emerald-400"}`}
                          >
                            {lift.paused
                              ? "Paused"
                              : `+${lift.delta6Wk} ${settings.units}`}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      {lift.paused && (
                        <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center border border-[#1a1a1a] z-10 p-4 text-center">
                          ...
                          <p className="text-[11px] text-neutral-500 mt-2 max-w-sm">
                            {lift.pauseReason}
                          </p>
                        </div>
                      )}
                      <div className="bg-black border border-[#1a1a1a] h-32 w-full pt-4">
                        <svg viewBox="0 0 400 100" className="w-full h-full">
                          ...
                          {lift.history.map((val, idx) => {
                            if (!val) return null;
                            const x =
                              (idx / (lift.history.length - 1)) * 360 + 20;
                            const y =
                              80 - ((val - minVal) / (maxVal - minVal)) * 60;
                            return (
                              <circle
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {injuries.map((inj) => (
                  <div
                    key={inj.id}
                    className="bg-[#0c0c0c] border border-[#1a1a1a] p-4 flex justify-between items-start"
                  >
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <span
                          className={`text-[8px] font-mono px-1.5 uppercase font-bold border
                        ...
                        <h3 className="text-xs font-bold uppercase tracking-tight text-neutral-200 font-mono">
                          {inj.name}
                        </h3>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1 pt-1">
                        {inj.forbiddenTags.map((t, idx) => (
                          <span
                            key={idx}
                            className="text-[8px] font-mono bg-black text-neutral-400 border border-[#1a1a1a] px-1.5 py-0.5 uppercase tracking-wider"
                          >
                            {t.replace(/_/g, " ")}
                          </span>
                        ))}
              ...
              <div className="bg-[#0c0c0c] border border-[#1a1a1a] divide-y divide-[#1a1a1a]">
                  ...
                  <div className="flex border border-[#1a1a1a] bg-black p-0.5 font-mono">
                    {[60, 90, 120, 150].map((sec) => (
                      <button
                        key={sec}
                        onClick={() =>
                          setSettings({ ...settings, defaultRest: sec })
                        }
                  ...
                  <div className="flex border border-[#1a1a1a] bg-black p-0.5 font-mono">
                    {["RPE", "RIR"].map((mode) => (
                      <button
                        key={mode}
                        onClick={() =>
                          setSettings({ ...settings, rpeMode: mode })
                        }
                ...
                <div className="p-4 flex justify-between items-center">
                  <div>
                    ...
                    <span className="text-[10px] font-mono text-neutral-500">
                      Auto-saves locally after each session
                    </span>
                  </div>
      ...
      {showRestBanner && (
        <div className="fixed bottom-16 left-0 right-0 bg-[#0c0c0c] border-t border-[#1a1a1a] px-4 py-3 flex items-center justify-between z-30">
          <div className="flex items-center space-x-3">
            <Timer
              className={`h-4 w-4 ${restRunning ? "text-blue-500 animate-pulse" : "text-neutral-500"}`}
            />
            <div>
              <span className="text-[8px] font-mono text-neutral-500 uppercase block tracking-wider leading-none">
                Rest
              </span>
              <span
                className={`text-xs font-mono font-bold leading-none ${restSeconds === 0 ? "text-emerald-400 animate-pulse" : "text-neutral-200"}`}
              >
                {restSeconds === 0
                  ? "Done — load next set"
          ...
          <div className="flex items-center space-x-1 font-mono">
            <button
              onClick={() => setRestSeconds((p) => p + 30)}
              className="bg-[#121212] hover:bg-[#1a1a1a] border border-[#222] text-neutral-300 text-[9px] px-2 py-1"
            >
              +30s
            </button>
            <button
              onClick={() => setRestSeconds((p) => Math.max(0, p - 10))}
              className="bg-[#121212] hover:bg-[#1a1a1a] border border-[#222] text-neutral-300 text-[9px] px-2 py-1"
            >
              −10s
            </button>
            <button
              onClick={() => setRestRunning((p) => !p)}
              className="bg-[#121212] hover:bg-[#1a1a1a] border border-[#222] text-neutral-300 text-[9px] px-2.5 py-1"
            >
            ...
            <button
              ...
              className="bg-[#121212] hover:bg-[#1a1a1a] border border-[#222] text-neutral-300 text-[9px] p-1"
            >
              <RotateCcw className="h-3 w-3" />
            </button>
            <button
              onClick={() => setShowRestBanner(false)}
              className="text-neutral-500 hover:text-white pl-2"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50">
          <div className="bg-[#0c0c0c] border border-[#1a1a1a] w-full max-w-md p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-[#1a1a1a] pb-3">
              <h3 className="text-xs font-bold uppercase tracking-widest font-mono text-neutral-300">
                Log injury
              </h3>
              <button
            ...
            <div className="space-y-4 font-mono text-xs">
              <div className="space-y-1">
                <label className="text-neutral-500 uppercase text-[9px] tracking-widest block">
                  Injury name
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setInjuryForm({ ...injuryForm, severity: e.target.value })
                  }
                  className="w-full bg-black border border-[#1a1a1a] px-3 py-2 text-white outline-none focus:border-blue-500 rounded-none text-xs"
                >
